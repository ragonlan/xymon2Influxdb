...
def getConfig(hostname, desc):
    try:
        with open("/etc/xymon/xymon2Influxdb.toml") as conffile:
            configDetails = toml.loads(conffile.read())
    except e:
    ...
    completeDetails[hostname].update(defaultConfig[hostname])
    completeDetails[hostname].update(configDetails[hostname])
